...
function beginV3PaymentRequest(){
  ...
  function contactFormValidation(ev){
    var requirementsCheck = [];
    $('.required1').each(function(){
      if ($(this).val() == "") {
        requirementsCheck.push(1);
        $(this).css('border-color', 'rgb(185, 21, 23)').css('border-width', '3px');
      }
    });

    if (requirementsCheck.includes(1)) {
      // Send the token to your server to charge it!
      finalizeOrder(ev);
    } else {
      window.paymentRequest.abort();
    }
  }
  ...
  function finalizeOrder(result) {
    ...
    // Only populate the form fields from ApplePay if formData values are blank
    ...
    if(result.payerEmail != null && result.payerEmail != ''){
      if ($('#cfAR input[name="contact[email]"]').val() == '') {
    ...
    if(result.payerName == null){
      if(result.shippingAddress != null){
        if ($('#cfAR input[name="contact[name]"]').val() == '') {
    ...
    if(result.phoneNumber != null && result.phoneNumber != ''){
      if ($('#cfAR input[name="contact[phone]"]').val() == '') {
    ...
    if(result.shippingAddress && result.shippingAddress.addressLine != ''){
      if ($('#cfAR input[name="contact[shipping_address]"]').val() == '') {
    ...
    if(result.shippingAddress && result.shippingAddress.city != ''){
      if ($('#cfAR input[name="contact[shipping_city]"]').val() == '') {
    ...
    if(result.shippingAddress && result.shippingAddress.region != ''){
      if ($('#cfAR input[name="contact[shipping_state]"]').val() == '') {
    ...
    if(result.shippingAddress && result.shippingAddress.postalCode != ''){
      if ($('#cfAR input[name="contact[shipping_zip]"]').val() == '') {
    ...
    if(result.shippingAddress && result.shippingAddress.country != ''){
      if ($('#cfAR input[name="contact[shipping_country]"]').val() == '') {
  ...
  window.paymentRequest.on('token', function(ev) {
    $('.otoloading').show();
    // Validate the form input before trying to process anything
    contactFormValidation(ev)
  });
}
...
function beginApplePay() {
  ...
  $('#cfAR input[name="purchase[product_ids][]"][data-product-currency-code]:checked').each(function(){
    var productAmt = parseFloat( Number( $(this).data('product-amount') ).toFixed(2) );
  ...
  // Can we check all of the $('.required1') fields and if any of them do not have a value
  // can we then stop the function and show an error?

  // Or better yet, can we disable the apple pay / google pay button if the $('.required1') fields 
  // don't have values?
  ...
  var session = Stripe.applePay.buildSession(window.paymentRequest,
    ...
    if (requiredFieldsCheck.includes(1)) {
